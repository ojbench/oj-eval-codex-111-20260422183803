...
class ACMOJClient:
    ...
    def submit_code(self, problem_id: int, code: str, language: str = "c++") -> Optional[Dict]:
        # Submit raw source code directly
...
def main():
    ...
    code_parser = subparsers.add_parser("submit-code", help="Submit raw code from a file")
    code_parser.add_argument("--problem-id", type=int, required=True, help="Problem ID")
    code_parser.add_argument("--file", type=str, required=True, help="Path to code file to submit (e.g., src.hpp)")
    code_parser.add_argument("--language", type=str, default="c++", help="Language (default: c++)")
